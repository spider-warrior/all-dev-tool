...
/**
 * @author yj
 * @since 2019-12-13 13:19
 **/
public class PseudoJedisCluster implements JedisCommands, MultiKeyJedisClusterCommands, JedisClusterScriptingCommands, Closeable {

    private int maxAttempts = 2;
    private JedisClusterConnectionHandler connectionHandler;

    public PseudoJedisCluster(HostAndPort hostAndPort, int timeout) {
        connectionHandler = new PseudoJedisClusterConnectionHandler(hostAndPort, timeout);
    }

    public PseudoJedisCluster(HostAndPort hostAndPort, int connectionTimeout, int soTimeout) {
        connectionHandler = new PseudoJedisClusterConnectionHandler(hostAndPort, connectionTimeout, soTimeout);
    }

    public PseudoJedisCluster(HostAndPort hostAndPort, int connectionTimeout, int soTimeout, String password) {
        connectionHandler = new PseudoJedisClusterConnectionHandler(hostAndPort, connectionTimeout, soTimeout, password);
    }
    ...
    public ScanResult<String> scan(final String cursor, final ScanParams params) {
        throw new RuntimeException("未实现该方法(该类不能直接从JedisCluster中复制实现方法)");
    }
    ...
    public ScanResult<Map.Entry<String, String>> hscan(final String key, final String cursor) {
        return new JedisClusterCommand<ScanResult<Map.Entry<String, String>>>(connectionHandler,
            maxAttempts) {
            @Override
